...
parse_metrics(Metrics, Resource, #{add_target_info:=AddTargetInfo,order:=Order} = Opts) ->
    ...
    MetricKeys =
        case Order of
            ordered -> lists:sort(fun(A, B) -> A < B end, maps:keys(ParsedMetrics1));
            reversed -> lists:sort(fun(A, B) -> A > B end, maps:keys(ParsedMetrics1));
            _ -> maps:keys(ParsedMetrics1)
        end,
    lists:map(fun(K) -> maps:get(K, ParsedMetrics1) end, MetricKeys).
...
fix_unit(Unit) ->
    case string:split(Unit, "/", all) of
        [_] -> iolist_to_binary([guess_unit(Unit)]);
        List ->
...
%% https://github.com/open-telemetry/opentelemetry-go/blob/main/exporters/prometheus/exporter.go#L369

%% Time
try_unit(<<"d">>) -> <<"days">>;
try_unit(<<"h">>) -> <<"hours">>;
try_unit(<<"min">>) -> <<"minutes">>;
...
datapoint(#datapoint{} = DP, MetricName, AddCreated, ScopeLabels, [Points, Created]) ->
    ...
    Point = [MetricName, Labels, ?SP, number_to_binary(DP#datapoint.value), ?LF],
