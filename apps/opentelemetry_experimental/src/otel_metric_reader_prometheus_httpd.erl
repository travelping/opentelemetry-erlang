...
do(#mod{method="GET",request_uri="/metrics",config_db=ConfigDb}) ->
    ...
    Config = ets:lookup_element(ConfigDb, otel_config, 2),
    Serializer = fun(Metrics, Resource) ->
                         otel_metric_serializer_prometheus:serialize(Metrics, Resource, Config)
                 end,
    Metrics = otel_metric_reader:collect(ReaderPid, Serializer),
    Headers = [
        {code, 200},
