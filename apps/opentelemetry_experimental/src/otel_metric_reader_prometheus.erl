...
start_link(ReaderId, ProviderSup, Config0) ->
    ...
    Config = Config1#{default_temporality_mapping => ?TEMPORALITY_MAPPING},

    case Config of
        #{server_name := ServerName} ->
            gen_server:start_link({local, ServerName}, otel_metric_reader,
...
call_collect(ReaderPid) ->
    otel_metric_reader:call_collect(ReaderPid).
